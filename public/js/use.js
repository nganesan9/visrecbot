...
function setupUse(params) {
  ...
  function showResult(results) {
    ...
    var image_to_be_sent_to_poker_classifier = $(outputImage).attr('src')
    alert("Image to be sent to poker classifier : " + image_to_be_sent_to_poker_classifier);

    scrollToElement($result);
    alert("Done classifying");

    //Call $.post here to classify poker image.. !!! THIS IS PROGRESS :) :)
    $.ajax({
			url: "/getpokerclassifieranswer",
			type: "GET",
            dataType: "json",
			contentType: "application/json",
			data: {image_path: image_to_be_sent_to_poker_classifier},
			cache: true,
			timeout: 5000,
      async: false,
      complete: function() {
				  //called when complete
				  alert("Done receiving all the data from Poker classifier");
				},
      success: function(data) {
          var class_name = JSON.stringify(data.class_name);
          var score = JSON.stringify(data.score);
          var image_path = JSON.stringify(data.image_path);
          image_path = image_path.substring(1, image_path.length-1);
          alert("Class is : " + class_name + "Score is : " + score + "Image path is : " + image_path);

          //Setting image source and output
          $("#poker_identification_result").html("<p> Class = " + class_name + "...... Score = " + score + "</p>");
        },
  ...
  function classifyImage(imgPath, imageData, beforeFunction, afterFunction) {
    ...
    // Grab all form data
    alert("Image path before call is : " + imgPath);
